...
func getVMClient(ctx context.Context) (*armcompute.VirtualMachinesClient, error) {
	...
	tenantID := cloudy.DefaultEnvManager.GetVar("VMC_AZ_TENANT_ID")
	clientID := cloudy.DefaultEnvManager.GetVar("VMC_AZ_CLIENT_ID")
	clientSecret := cloudy.DefaultEnvManager.GetVar("VMC_AZ_CLIENT_SECRET")
	subscriptionID := cloudy.DefaultEnvManager.GetVar("VMC_AZ_SUBSCRIPTION_ID")
...
func VmList(ctx context.Context, vmClient *armcompute.VirtualMachinesClient, rg string) ([]*cloudyvm.VirtualMachineStatus, error) {
	...
	// resourceGroup, _ := cloudy.DefaultEnvManager.GetVar("AZ_RESOURCE_GROUP")
